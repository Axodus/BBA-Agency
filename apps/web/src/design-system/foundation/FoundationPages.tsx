...
export const surfaces = {
  ...
  packages: { eyebrow: "Distribution preparation", title: "Distribution Packages", description: "Sets of Channel Variants prepared for distribution without assuming external publication.", canonicalOwner: "Steward", emptyTitle: "No Distribution Package created", emptyDescription: "Packages can only be composed from approved Channel Variants.", record: { type: "Distribution Package", id: "package-linkedin-brief-not-constituted", label: "DP — LinkedIn Brief Set 2026", state: "blocked", stateLabel: "Blocked", constraint: "Preparation does not constitute external publication." } },
...
export function SurfaceTemplate({ surface }: { surface: FoundationSurface }) {
  return <div className="foundation-page">
    <PageHeader eyebrow={surface.eyebrow} title={surface.title} description={surface.description} actions={<Button aria-label="New record unavailable in local reference" disabled title="Record creation requires a configured runtime."><Plus size={16} /> New record</Button>} />
    ...
    <Panel eyebrow="Empty state" title="Handled absence"><EmptyState title={surface.emptyTitle}><p>{surface.emptyDescription}</p></EmptyState></Panel>
...
export function AccountPage() {
  return <div className="foundation-page"><PageHeader eyebrow="Account" title="Steward profile" description="Personal preferences and operating context. Real authentication remains outside this scope." />
    <p>Profile is read-only. Saving preferences is unavailable.</p>
    <div className="foundation-form-grid"><Panel eyebrow="Local identity" title="Profile data"><form className="foundation-form" onSubmit={(event) => event.preventDefault()}><Field label="Name"><Input readOnly defaultValue="Ana Lemos" /></Field><Field label="Role"><Input readOnly defaultValue="Institutional Steward" /></Field><Field label="Demo email" hint="Not used for authentication."><Input readOnly type="email" defaultValue="ana.lemos@example.invalid" /></Field><Button disabled>Save preferences</Button></form></Panel><Panel eyebrow="Prepared permissions" title="Governance scope"><ul className="foundation-permission-list"><li><Check size={17} />Review Institutional Assets</li><li><Check size={17} />Record decisions in the Audit Record</li><li><LockSimple size={17} />Manage Connectors — unavailable</li></ul></Panel></div>
  </div>;
}
...
export function SettingsPage() {
  ...
  return <div className="foundation-page"><PageHeader eyebrow="Settings" title="Application preferences" description="Local interface, governance, and notification settings without secrets or infrastructure configuration." />
    <p>Settings are read-only. Saving preferences and notification delivery are unavailable.</p>
    <Tabs label="Settings sections" activeId={activeTab} onChange={setActiveTab} items={[{ id: "interface", label: "Interface", content: <form className="foundation-settings-panel" onSubmit={(event) => event.preventDefault()}><Field label="Density"><Select defaultValue="comfortable"><option value="comfortable">Comfortable (fixed)</option></Select></Field><Field label="Language"><Select defaultValue="en-US"><option value="en-US">English (United States)</option></Select></Field><Button disabled>Save settings</Button></form> }, { id: "governance", label: "Governance", content: <Feedback title="Institutional policies">Rules are display-only in this reference and cannot be changed through this interface.</Feedback> }, { id: "notifications", label: "Notifications", content: <form className="foundation-settings-panel" onSubmit={(event) => event.preventDefault()}><Checkbox disabled defaultChecked label="Notify me when a Steward decision is pending" /><Checkbox disabled defaultChecked label="Notify me when a local execution fails" /><Checkbox disabled label="Summarize AI Workforce updates" /><Button disabled variant="secondary">Save notification preferences</Button></form> }]} />
  </div>;
}
