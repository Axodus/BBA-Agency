...
import {
  Button, EmptyState, Feedback, Field, Input, Modal, Panel, Select, Skeleton, StatusBadge,
...
export const surfaces = {
  assets: { eyebrow: "Biblioteca canônica", title: "Institutional Assets", description: "Registros institucionais controlados, rastreáveis e derivados de uma Mission.", canonicalOwner: "Human Governance", emptyTitle: "Nenhum Institutional Asset neste recorte", emptyDescription: "Ajuste os filtros ou inicie uma Mission para produzir um ativo sob governança." },
  packages: { eyebrow: "Preparação de distribuição", title: "Distribution Packages", description: "Conjuntos de Channel Variants preparados para distribuição, sem pressupor publicação externa.", canonicalOwner: "Steward", emptyTitle: "Nenhum Distribution Package constituído", emptyDescription: "Pacotes só podem ser compostos a partir de Channel Variants aprovadas." },
  governance: { eyebrow: "Controle humano", title: "Governança", description: "Fila de decisões, políticas aplicáveis e Audit Records das superfícies BBA.", canonicalOwner: "Steward", emptyTitle: "Nenhuma decisão pendente", emptyDescription: "Novas solicitações aparecerão quando a AI Workforce concluir uma etapa governada." },
...
export function FoundationOverview() {
  return <div className="foundation-page">
    <PageHeader eyebrow="BBA App UI Foundation" title="Trabalho de IA sob governança humana" description="Uma base visual e estrutural para Missions, Institutional Assets, decisões e registros auditáveis." actions={<Link className="foundation-primary-link" to="/foundation/missions/msn-024">Abrir Mission Workspace <ArrowRight size={17} /></Link>} />
    ...
    <div className="foundation-overview-grid">
      <Panel eyebrow="Referência do produto" title="Mission em foco" action={<StatusBadge state="awaiting">Aguardando decisão</StatusBadge>}><h3>Clareza institucional para o próximo ciclo</h3><p>Estabelecer uma narrativa institucional clara para orientar decisões e comunicações no ciclo 2026.3.</p><Link className="foundation-inline-link" to="/foundation/missions/msn-024">Ver workspace <ArrowRight size={15} /></Link></Panel>
...
export function SurfaceTemplate({ surface }: { surface: FoundationSurface }) {
  return <div className="foundation-page">
    ...
    <Panel eyebrow="Estado de referência" title="Superfície preparada"><EmptyState title={surface.emptyTitle}><p>{surface.emptyDescription}</p><Button variant="secondary">Entender o contrato</Button></EmptyState></Panel>
...
export function UiKitPage() {
  ...
  return <div className="foundation-page"><PageHeader eyebrow="Design System / UI SDK" title="UI Kit BBA" description="Tokens, estados e componentes desacoplados de endpoints, prontos para reutilização nas superfícies do app." />
    <Panel eyebrow="Semântica" title="Estados de domínio"><div className="foundation-component-row">{(Object.entries(semanticStateLabels) as Array<[SemanticState, string]>).map(([state, label]) => <StatusBadge state={state} key={state}>{label}</StatusBadge>)}</div></Panel>
