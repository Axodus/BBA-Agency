...
export function DevSessionSetup({ initialBaseUrl, onConfigure }: { readonly initialBaseUrl: string; onConfigure(configuration: DevSessionConfiguration): void }) {
  ...
  function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try { new URL(configuration.baseUrl); } catch { setError("Enter a valid absolute API URL."); return; }
    onConfigure(configuration);
  }
  return <main className="bba-setup" id="main-content"><Card><span className="bba-page__eyebrow">Development environment</span><h1>Configure ephemeral session</h1><p>Values remain in memory only and are cleared when the page reloads.</p>{error === undefined ? null : <Alert title="Invalid configuration">{error}</Alert>}<form className="bba-form" onSubmit={submit}><Field id="baseUrl" label="API base URL"><Input defaultValue={initialBaseUrl} id="baseUrl" name="baseUrl" placeholder="http://localhost:3000" /></Field><Field id="accessToken" label="Bearer token"><Input autoComplete="off" id="accessToken" name="accessToken" type="password" /></Field><Field id="tenantId" label="Tenant"><Input id="tenantId" name="tenantId" /></Field><Field id="subject" label="Subject"><Input id="subject" name="subject" /></Field><Field id="actorReference" label="Actor reference"><Input id="actorReference" name="actorReference" /></Field><Button type="submit">Start local session</Button></form></Card></main>;
}
