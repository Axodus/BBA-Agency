...
export class Orchestrator {
  ...
  async run(config: OrchestratorConfig): Promise<CampaignContext> {
    ...
    const interpretedData = interpreted.data as unknown as InterpretedBrief;
    ctx = {
      ...ctx,
    ...
    ctx = { ...ctx, icp: audience.data as unknown as AudienceProfile };
    ...
    const validationData = validation.data as unknown as ValidationSummary;
    ...
    const copyData = copy.data as unknown as CopyBundle;
    ctx = {
      ...ctx,
      assets: [
        {
    ...
    if (config.metrics) {
      ...
      const feedback = await this.agents.analytics.run(ctx);
      ctx = { ...ctx, feedback: feedback.data as any };
    }
