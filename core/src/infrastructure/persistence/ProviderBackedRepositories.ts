...
export class ProviderBackedRepository<TAggregate extends PersistableAggregate, TSnapshot> {
  public constructor(protected readonly provider: PersistenceProviderPort, protected readonly aggregateCodec: AggregateSnapshotCodec<TAggregate, TSnapshot>, protected readonly context?: TransactionContext) {}
  public withContext(context: TransactionContext): ProviderBackedRepository<TAggregate, TSnapshot> { return new ProviderBackedRepository(this.provider, this.aggregateCodec, context); }
  public async save(aggregate: TAggregate, expectedVersion: Version): Promise<void> { const context = this.requireContext(); const uow = this.provider.begin(context); uow.stage(aggregate, this.aggregateCodec, expectedVersion.value); await uow.commit(); }
