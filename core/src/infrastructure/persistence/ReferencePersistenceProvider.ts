...
interface ProviderState { readonly snapshots: Map<string, SnapshotRecord>; readonly events: Map<string, readonly PersistedEvent[]>; readonly audits: readonly AuditRecord[]; readonly outbox: readonly OutboxMessage[]; readonly outcomes: Map<string, { readonly fingerprint: string; readonly payloadFingerprint: CanonicalPayloadDescriptor | null; readonly transactionSequence: number }>; readonly nextTransactionSequence: number; }
...
export class ReferencePersistenceProvider implements PersistenceProviderPort, SnapshotStorePort, EventStorePort {
  ...
  public getTransactionFingerprint(transactionId: string): CanonicalPayloadDescriptor | null { return this.state.outcomes.get(transactionId)?.payloadFingerprint ?? null; }
  ...
  public applyCommit(context: TransactionContext, staged: readonly StagedAggregate[], fingerprint: string, payloadFingerprint: CanonicalPayloadDescriptor | null = null): void {
    ...
    this.state = { snapshots, events, audits, outbox, outcomes: new Map(this.state.outcomes).set(context.transactionId, { fingerprint, payloadFingerprint, transactionSequence }), nextTransactionSequence: transactionSequence };
...
class ReferenceUnitOfWork implements UnitOfWorkPort {
  ...
  public stage<TAggregate, TSnapshot>(aggregate: TAggregate & PersistableAggregate, codec: AggregateSnapshotCodec<TAggregate, TSnapshot>, expectedVersion: number): void {
    ...
    const events = aggregate.domainEvents.map((event) => event); const currentEvents = this.provider.getEvents(aggregateType, tenantId, aggregateId); const currentSnapshot = this.provider.getSnapshot(aggregateKey);
  ...
  public async commit(payloadFingerprint?: CanonicalPayloadDescriptor): Promise<void> { if (this.state === "COMMITTED") return; if (this.state === "ROLLED_BACK") throw new PersistenceFailure("UnitOfWork was rolled back"); const items = [...this.staged.values()]; const fingerprint = stableSerialize({ context: this.context.toJSON(), payloadFingerprint: payloadFingerprint ?? null, items: items.map((item) => ({ aggregateType: item.aggregateType, aggregateId: item.aggregateId, tenantId: item.tenantId, expectedVersion: item.expectedVersion, snapshot: item.snapshot, events: item.events.map((event) => event.toJSON()) })).sort((a, b) => `${a.tenantId}|${a.aggregateType}|${a.aggregateId}`.localeCompare(`${b.tenantId}|${b.aggregateType}|${b.aggregateId}`)) } as unknown as JsonObject); this.provider.applyCommit(this.context, items, fingerprint, payloadFingerprint ?? null); this.state = "COMMITTED"; }
