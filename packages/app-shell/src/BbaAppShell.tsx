...
function RuntimeBoundary({ runtime }: { readonly runtime: ShellRuntimeState }) {
  if (runtime.status === "LOADING") return <main className="bba-shell-state" id="main-content"><Spinner label="Preparing session" /></main>;
  if (runtime.status === "CONFIGURATION_MISSING") return <main className="bba-shell-state" id="main-content"><Card><Alert title="Configuration missing">{runtime.message}</Alert></Card></main>;
  if (runtime.status === "SESSION_ERROR") return <main className="bba-shell-state" id="main-content"><Card><Alert title="Unable to prepare session">{runtime.message}</Alert></Card></main>;
  return null;
}
