...
export function ConfirmationDialog({ trigger, title, description, confirmLabel, onConfirm }: { readonly trigger: ReactNode; readonly title: string; readonly description: ReactNode; readonly confirmLabel: string; onConfirm(): void }) {
  return <AlertDialog.Root><AlertDialog.Trigger asChild>{trigger}</AlertDialog.Trigger><AlertDialog.Portal><AlertDialog.Overlay className="bba-drawer__overlay" /><AlertDialog.Content className="bba-confirmation"><AlertDialog.Title>{title}</AlertDialog.Title><AlertDialog.Description asChild><div>{description}</div></AlertDialog.Description><div className="bba-confirmation__actions"><AlertDialog.Cancel asChild><Button variant="secondary">Back</Button></AlertDialog.Cancel><AlertDialog.Action asChild><Button onClick={onConfirm}>{confirmLabel}</Button></AlertDialog.Action></div></AlertDialog.Content></AlertDialog.Portal></AlertDialog.Root>;
}
