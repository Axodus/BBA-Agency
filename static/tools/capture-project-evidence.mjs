...
const evidenceRoot = resolve(repositoryRoot, ".rag/evidence/REQ-IMP-019-FE-001");
...
for (const directory of ["desktop", "laptop", "tablet", "mobile", "route-smoke", "accessibility"]) {
...
try {
  ...
  for (const [viewportName, width, height] of viewports) {
    ...
    for (const [name, route] of routes) {
      ...
      const [h1Count, mainCount, overflows] = await page.evaluate(() => [document.querySelectorAll("h1").length, document.querySelectorAll("main").length, document.documentElement.scrollWidth > window.innerWidth]);
    ...
    if (viewportName === "desktop") {
      for (const [name, route] of [["projects-new-fallback", "/projects/new"], ["projects-unknown-fallback", "/projects/unknown"]]) {
        ...
        await page.screenshot({ path: resolve(evidenceRoot, "route-smoke", `${name}.png`), fullPage: true, animations: "disabled" });
