...
export class ByokAgentExecutor implements AgentExecutorPort {
  ...
  public async execute<T>(definition: AgentDefinition, input: unknown, context: { readonly projectId: string; readonly channelId?: PublisherChannelId | undefined }): Promise<AgentExecutionResult<T>> {
    ...
    const output = this.credential.provider === "OPENAI" ? await this.openai(prompt(definition, input, context.channelId)) : await this.anthropic(prompt(definition, input, context.channelId));
    const schema = schemaFor(definition.id); if (!schema) throw new Error("AGENT_OUTPUT_SCHEMA_MISSING");
    return { output: schema.parse(extractJson(output)) as T, metadata: { provider: this.credential.provider as LlmProvider, model: this.credential.model, durationMs: Date.now() - started } };
  }
  private async openai(content: string) { const response = await this.request("https://api.openai.com/v1/responses", { method: "POST", headers: { authorization: `Bearer ${this.credential.apiKey}`, "content-type": "application/json" }, body: JSON.stringify({ model: this.credential.model, input: content }) }); if (!response.ok) throw new Error(`PROVIDER_FAILURE:${response.status}`); const body = await response.json() as { output_text?: string }; if (!body.output_text) throw new Error("PROVIDER_RESPONSE_INVALID"); return body.output_text; }
  private async anthropic(content: string) { const response = await this.request("https://api.anthropic.com/v1/messages", { method: "POST", headers: { "x-api-key": this.credential.apiKey, "anthropic-version": "2023-06-01", "content-type": "application/json" }, body: JSON.stringify({ model: this.credential.model, max_tokens: 4096, messages: [{ role: "user", content }] }) }); if (!response.ok) throw new Error(`PROVIDER_FAILURE:${response.status}`); const body = await response.json() as { content?: Array<{ type: string; text?: string }> }; const text = body.content?.find((item) => item.type === "text")?.text; if (!text) throw new Error("PROVIDER_RESPONSE_INVALID"); return text; }
}
